...
// moc_qparticleviewer.cpp is generated by moc at compile time
#include "moc_qparticleviewer.cpp"

QParticleViewer::QParticleViewer(QWidget * parent, const char * name, Qt::WindowFlags f, GridSlamProcessorThread* thread) :
    QWidget(parent, f)
{
...
void QParticleViewer::drawFromMemory()
{
  ...
  for (unsigned int i = 0; i < particleSize; i++) {
    ...
    bool done = false;
    for (GridSlamProcessorThread::EventDeque::reverse_iterator it = history.rbegin(); it != history.rend() && !done;
        it++) {
      GridSlamProcessorThread::ParticleMoveEvent* move = dynamic_cast<GridSlamProcessorThread::ParticleMoveEvent*>(*it);
